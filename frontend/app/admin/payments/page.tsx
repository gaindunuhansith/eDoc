...
import { useGetAllPayments, type PaymentStatus } from "@/api/paymentApi";
...
export default function AdminPaymentsPage() {
  ...
  const { data, isLoading, isError } = useGetAllPayments(page, PAGE_SIZE);
      ...
      <div className="flex flex-col space-y-6 w-full">
        <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-4 w-full">
          <div className="flex flex-wrap items-center gap-3 w-full lg:w-auto">
            <div className="relative w-full sm:w-80">
              ...
              <Input
                placeholder="Search by User ID, Order ID, or Appointment"
                value={search}
                ...
                <TableRow>
                  <TableCell colSpan={7} className="py-16 text-center">
                    <div className="flex flex-col items-center gap-2 text-muted-foreground">
                      ...
                      <p className="text-sm">Failed to load payments. Please try again.</p>
                    </div>
                  </TableCell>
                </TableRow>
              )}
              ...
              {!isLoading && !isError && filtered.map((tx) => (
                ...
                >
                  ...
                  <TableCell className="py-4">
                    <p
                      className="text-sm font-mono text-foreground truncate max-w-35"
                      title={tx.userId}
                    >
                  ...
                  <TableCell className="py-4">
                    <div className="text-sm font-medium text-foreground font-mono" title={tx.id}>
                      {truncateId(tx.id)}
                    </div>
                    {tx.orderId && (
                      <div className="text-xs text-muted-foreground mt-0.5">{tx.orderId}</div>
                    )}
                  </TableCell>
                  <TableCell className="py-4">
        ...
        <div className="flex flex-col sm:flex-row justify-between items-center gap-4 pt-4 border-t border-border/40">
          <p className="text-sm text-muted-foreground">
            {totalElements === 0
              ? "No payments"
              : <>Showing <span className="font-medium text-foreground">{startRow}–{endRow}</span> of <span className="font-medium text-foreground">{totalElements}</span> payments</>
            }
          </p>
